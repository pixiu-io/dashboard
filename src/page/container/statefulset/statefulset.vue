  ...
  <div style="margin-top: 25px">
    <el-row>
      <el-col>
        ...
        >
          <template #suffix>
            <el-icon class="el-input__icon" @click="getStatefulsets">
              <component :is="'Search'" />
            </el-icon>
          </template>
        </el-input>
            ...
            <el-dropdown>
              ...
              <template #dropdown>
                <el-dropdown-menu class="dropdown-buttons">
                  <el-dropdown-item style="color: #006eff" @click="deleteStatefulset(scope.row)">
                    删除
                  </el-dropdown-item>
                </el-dropdown-menu>
              </template>
...
//   const url = `/kubernetes/statefulset_create?cluster=${data.cluster}&namespace=${data.namespace}`;
//   router.push(url);
// };
...
const getStatefulsets = async () => {
  ...
  const res = await proxy.$http({
    method: 'get',
    url: `/proxy/pixiu/${data.cluster}/apis/apps/v1/namespaces/${data.namespace}/statefulsets`,
    data: data.pageInfo,
  });

  console.log(res.items);
  data.loading = false;
  data.statefulsetList = res.items;
